...
class RangePicker extends React.Component<RangePickerProps, RangePickerState> {
  ...
  static getDerivedStateFromProps(
    nextProps: RangePickerProps,
    prevState: RangePickerState,
  ) {
    let state = null;
    if ('value' in nextProps) {
      ...
      if (!shallowequal(nextProps.value, prevState.value)) {
        state = {
          ...state,
          showDate:
            getShowDateFromValue(value, nextProps.mode) || prevState.showDate,
        };
      }
    }
  ...
  constructor(props: any) {
    ...
    const pickerValue =
      !value || isEmptyArray(value) ? props.defaultPickerValue : value;
    this.state = {
      value,
  ...
  handleChange = (value: RangePickerValue) => {
    ...
    if (typeof props.onChange === 'function') {
      props.onChange(value, [
        formatDate(start, props.format),
        formatDate(end, props.format),
      ]);
    }
  };
  ...
  handleShowDateChange = (showDate: RangePickerValue) =>
    this.setState({ showDate });
  ...
  renderRangePicker = ({ getPrefixCls }: ConfigConsumerProps) => {
    ...
    const prefixCls = getPrefixCls('legacy-calendar', customizePrefixCls);
    ...
    const inputIcon = (
      <InputIcon suffixIcon={suffixIcon} prefixCls={prefixCls} />
    );
      ...
      return (
        <span className={props.pickerInputClass}>
          <input
            ...
            tabIndex={-1}
          />
          <span className={`${prefixCls}-range-picker-separator`}>
            {' '}
            {separator}{' '}
          </span>
          <input
  ...
  render() {
    return <CompatibleConsumer>{this.renderRangePicker}</CompatibleConsumer>;
  }
}
